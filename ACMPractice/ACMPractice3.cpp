...
struct A {
	...
	void read() {
		rep(i, 0, 4)
			cin >> x[i] >> y[i];
		min_x = *min_element(x, x + 4);
		max_x = *max_element(x, x + 4);
		min_y = *min_element(y, y + 4);
		max_y = *max_element(y, y + 4);
	}
	bool in(int x, int y) {
...
struct B {
	...
	bool in(int x, int y) {
		int x0 = x - min_x, y0 = y - min_y;
		if ((x0 + y0 - this->y[1]) >= 0 && (x0 + y0 - (this->y[1] + this->y[3])) <= 0 && (-x0 + y0 + this->y[1]) >= 0 && (-x0 + y0 - this->y[1]) <= 0)
			return true;
		return false;
	}
};
...
/*
// CF 488 B Knights of a Polygonal Table
const int N = 1e5 + 5;
pair<ll, int> v[N];
ll ans[N], x[N];
...
void dfs(int x, int y, int cnt) {
	...
	if (x == a.size() - 1) {
		int c = sqrt(y);
		if (c*c == y && y != 0) ans = min(ans, cnt + 1);
		y = y * 10 + b;
		c = sqrt(y);
		if (c*c == y && y != 0) ans = min(ans, cnt);
		return;
	}
	if (b != 0 || y != 0)
		dfs(x + 1, y * 10 + b, cnt);

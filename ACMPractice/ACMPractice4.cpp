...
struct seg {
	int l, r;
	bool operator<(seg & oth) {
		if (l == oth.l) return r > oth.r;
		return l < oth.l;
	}
	seg operator|(seg & oth) {
		return seg{ max(l,oth.l), min(r, oth.r) };
	}
} s[N], pre[N], suf[N];
...
int main()
{
	...
	int ans = 0;
	repn(i, n) {
		seg now = pre[i - 1] | suf[i + 1];
		ans = max(ans, now.r - now.l);
	}
